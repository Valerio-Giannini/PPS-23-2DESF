...
object WorldJS:
  def apply(): WorldTrait = WorldJSImpl()
  
private case class WorldJSImpl() extends WorldTrait:
  ...
  def getComponent[T <: Component](entity: Entity)(using tag: scala.reflect.ClassTag[T]): Option[T] =
    entitiesToComponents.get(entity) match
